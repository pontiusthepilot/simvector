...
//      Change the pisition of the lattitude and longitude labels.
...
var Graticule = (function() {
    ...
    _.prototype.draw = function() {
        ...
        // labels on first, second or third column depending on zoom level
        if (this.map.getZoom() > 9) {
            ...
            var y = latLngToPixel(this, b + 3 * dLat, l).y + 2;
        }
        ...
        // labels on first, second or third row depending on zoom level
        if (this.map.getZoom() > 9) {
            ...
            var x = latLngToPixel(this, b, l + 3 * dLng).x + 3;
        }

        for (la = b; la <= t; la += dLat) {
            var py = latLngToPixel(this, la, l).y;
            this.addDiv(parallel(py, color));

            this.addDiv(makeLabel(color,
        x, py + 6,
        (this.sex_ ? this.decToLatSex(la) : la.toFixed(gridPrecision(dLat)))));
        }
